...
const updatePizza = (req, res)=>{
    ...
    // creamos la consulta
    const sql = 'UPDATE products SET productName = ?, price = ?, ingredients = ?, description = ?, img = ?, state = ? WHERE idProducts = ?';

    //Enviamos la consulta a la db
    db.query(sql,[name, price, ingredients,description,img,state,id],(err, result)=>{
        //si sucede algun error
        if(err){throw err}
        //si todo sale bien
